        ...
        returns (bool)
    {
        ...
        for (uint i = 0; i < proof_inputs.length; i++) {
            ...
            assembly {
                ...
                success := staticcall(sub(sendgas, 2000), 7, mul_input, 0x60, add(add_input, 0x40), 0x40)
            }
            require( success, "Failed to call ECMUL precompile" );
            sendgas = gasleft();
            assembly {
                // ECADD
                success := staticcall(sub(sendgas, 2000), 6, add_input, 0x80, add_input, 0x40)
            }
        ...
        sendgas = gasleft();
        assembly {
            success := staticcall(sub(sendgas, 2000), 8, input, 768, out, 0x20)
        }
